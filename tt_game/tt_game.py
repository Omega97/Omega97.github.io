...
# NUMBERS = tuple([f" {i}" for i in range(10)])
...
class Game:
    ...
    def __init__(self, players: Dict[str, list] = None, board_size=10,
                 random_seed=0, life_cap=3, heal_self_cost=2,
                 gift_heart_cost=2, upgrade_cost=3, capture_cost=5):
        """
    ...
    def increase_ap(self, token, amount=1):
        if amount < 0:
            self.check_has_ap(token, -amount)
    ...
    def distance(self, token_1: str, token_2: str) -> float:
    ...
    def is_in_range(self, token_1: str, token_2: str) -> bool:
    ...
    def spend_ap(self, token: str, amount: int):
        self.increase_ap(token, -amount)

    def is_player_eliminated(self, player: str) -> bool:
    ...
    def upgrade_token(self, token: str) -> str:
        """
        Target token increases life-cap by +1 and range by +1 (no free life).
        Cost: 3 AP
        ...
        self.tokens[token]["life_cap"] += 1
        self.tokens[token]["range"] += 1

        new_cap = self.tokens[token]["life_cap"]
        new_range = self.tokens[token]["range"]

        msg = (f"{token} upgraded! New life cap: {new_cap} {self._get_lifebar(token)}, "
    ...
    def set_board_size(self, size: int):
        ...
        self._init_tokens()
        return f"Board size set to {size}x{size} (tokens repositioned, RNG reset)"
...
def test(filepath="tt_commands.txt"):
    game = Game()
